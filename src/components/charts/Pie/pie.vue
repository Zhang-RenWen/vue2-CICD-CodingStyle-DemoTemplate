...
import mixinCharts from '@/components/charts/mixins/mixinCharts'
...
export default {
  ...
  methods: {
    ...
    setOption() {
      ...
      option.linearGradient = Object.assign(option.linearGradient, setlinearGradient)
      option.centerInfoOption = Object.assign(option.centerInfoOption, setcenterInfoOption)
    },

    cornerRadiusDonut(svg) {
      const vm = this
      const { data, acrFn, ascending, pieFn, initAnimation, initAnimationDuration, option } = vm
      const { length } = data
          ...
          .each(function getArc() {
            ...
            d3.select(this)
              ...
              .attrTween('d', function setInterpolate(d) {
                ...
                copyd.endAngle = ascending ? (endAngle -= adjustment) : (endAngle += adjustment)
    ...
    draw() {
      ...
      const radius = width >= height ? (height * totalScale) / 2 : (width * totalScale) / 2 // 半徑
        ...
        for (let i = 0; i < arr.length; i += 1) {
          ...
          if (element) {
            ...
            el.selectAll('.arc').each(function setLabel(d, j) {
              const condition = isLeft ? coordinateData.length - 1 - i === j : i === j
              if (condition) {
                d3.select(this)
                  .append('text')
                  .attr('class', 'arcText')
                  ...
                  .attr('x', thisTextSpace.x >= 0 ? thisTextSpace.x + 10 : thisTextSpace.x - 10)
                ...
                d3.select(this)
                  ...
                  .attr('d', function setCoordinates(d2) {
                    ...
                    const x3 = textSpace[spaceIndex] ? textSpace[spaceIndex].x : x2
                    const y3 = textSpace[spaceIndex] ? textSpace[spaceIndex].y : y2
      ...
      if (labelsDisplay) {
        ...
        const anchorQuantity =
          Math.floor((textAnchorScale * 2 * radius) / (labelFontSize * (labelBreak ? 2 : 1))) * 2
        ...
        const textSpaceIsEnoughLeft = dataPutInLeftSpace.length <= textSpace.length / 2
        const textSpaceIsEnoughRight = dataPutInRightSpace.length <= textSpace.length / 2
        ...
        setLabelAndCoordinate(dataPutInLeftSpace, !ascending, textSpace, coordinateData)
        setLabelAndCoordinate(dataPutInRightSpace.reverse(), !!ascending, textSpace, coordinateData)
      }
      ...
      g.append('path')
        ...
        .style('fill', function setColor(d) {
          // 生成漸層 和 色階 start
          const { open, startFrom, endFrom } = option.linearGradient
          const { color } = d.data
          if (open) {
      ...
      if (centerInfoDisplay && !vm.isIE()) {
        const { circleScale, circleBackground, fontSize, color, text } = option.centerInfoOption // 進場動畫持續時間 結束後掛載事件的延遲時間
        svg
      ...
      setTimeout(() => {
        ...
        el.selectAll('.arcPath').on('mousemove', function trigger(event) {
          // d3.event ^5.15.0
          // event ^6.1.1
          const e = d3.event || event
          const elWidth = document.querySelector(`#${vm.id}`).getBoundingClientRect().width
          const elheight = document.querySelector(`#${vm.id}`).getBoundingClientRect().height
          const elTop = document.querySelector(`#${vm.id}`).getBoundingClientRect().top // 元素到頂部距離
          const elLeft = document.querySelector(`#${vm.id}`).getBoundingClientRect().left // 元素到左側距離
          ...
          const outOfRangeAdjustment = outOfRange ? -tooltipsHeight : tooltipsHeight * 1.2
          tooltips.attr(
            'transform',
            `translate(${positionXInEl - halfWidth - minWidth / 2},${positionYInEl -
              halfHeight -
          ...
          if (data[vm.hoverArc]) {
            tooltips.select('.tooltipsDot').attr('fill', data[vm.hoverArc].color)
          ...
          tooltips.select('.tooltips-rect').attr('width', function getTooltipWidth() {
            ...
            const nameFontsLength = eltooltipsName.getBoundingClientRect().width
            const valueFontsLength = eltooltipsValue.getBoundingClientRect().width
            const dot = tooltipsDotRadius * 2
            const padding = left + right
            const autoWidth = valueFontsLength + nameFontsLength + dot + padding

            if (minWidth >= autoWidth) {
              el.select('.tooltipsValue').attr('x', 0 + minWidth - right)
              return minWidth
            }
            el.select('.tooltipsValue').attr('x', 0 + autoWidth - right + textMargin)
