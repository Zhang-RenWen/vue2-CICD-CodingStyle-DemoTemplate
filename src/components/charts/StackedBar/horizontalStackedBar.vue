...
export default {
  ...
  methods: {
    ...
    draw() {
      ...
      focus
        ...
        .each(function createElememtsAndSensor(d, i) {
          ...
          const leftRoundedRect = (px, py, pwidth, pheight, radius) => {
            return `M${px + radius},${py}h${pwidth - radius}v${pheight}h${radius -
              pwidth}a${radius},${radius} 0 0 1 ${-radius},${-radius}v${2 * radius -
          ...
          if (isLast) {
            hoverZonePathD = rightRoundedRect(widthSum, 0, thisWtdth, tooltipHeight, 0)
          }
          if (!isFirst && !isLast) {
            hoverZonePathD = rightRoundedRect(widthSum, 0, thisWtdth, tooltipHeight, 0)
          }
      ...
      // 以下準備製作 tooltips
      if (tooltips.open) {
        const { top, left, bottom, right, minWidth, minHeight, dotRadius, fontSize } = tooltips // standards  valueInTextMargin
        focus
          .append('g')
          .attr('class', 'tooltips')
          .style('opacity', 0)
        ...
        svg.selectAll('.hoverZone').on('mousemove', function trigger(event) {
          ...
          function getX() {
            return e.x - elLeft - (isOverHalf ? thisToolTipsWidth + 2 * ttpadding : 0)
          }
          eltooltips.attr('transform', `translate(${getX()},${0 - padding.top})`)
          ...
          eltooltips.selectAll('.tooltipsDot').style('fill', function setColor() {
            const { color } = getData
            return color
          })
          // 寬度重新調整
          eltooltips.select('.tooltips-rect').attr('width', function getTooltipWidth() {
            ...
            const autoWidth =
              valueFontsLength + nameFontsLength + dot * 2 + ttpadding + textValueMargin
