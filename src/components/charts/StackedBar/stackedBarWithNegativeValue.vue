<template>
  <div>
    <div :id="id" class="StackedBarWithNegativeValue chartBase" />
    <slot name="legend" />
  </div>
</template>
...
export default {
  name: 'StackedBarWithNegativeValue',
  mixins: [mixinCharts],
        ...
        .each(function createElememtsAndSensor(d, i) {
          ...
          for (let j = 0; j < keyLen; j += 1) {
            ...
            d3.select(this)
              ...
              .attr('y', isNegative ? y(0) : y(j === 0 ? allSum[i] : d[key]))
              .attr('width', barWidth)
      ...
      if (tooltips.open) {
        ...
        svg.selectAll('.hoverZone').on('mousemove', function trigger(event) {
          ...
          eltooltips.selectAll('.tooltipsValue').each(function setValue(d, i) {
            const key = allKey[i]
            d3.select(this).text(f(thisData[key]))
          })
