<template>
  <div>
    <div :id="id" class="StackedBarWithPercentage chartBase" />
    <slot name="legend" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setYAxis(y, focus, height, width) {
      ...
      const { fontSize, fontFamily, textAnchorY, fill, stroke, strokeWidth, opacity } = axis
      focus
        .append('g')
    ...
    draw() {
      ...
      for (let i = 0; i < keyLen; i += 1) {
        ...
        for (let j = 0; j < data[key].length; j += 1) {
          ...
          if (i === 0) {
            allSum.push([element.value])
          }
          if (i !== 0) {
            allSum[j][i] = element.value
          }
        }
      }
      ...
      allSum = allSum.map(arr => {
        return SumDatareduce(arr)
      })
      /// ///////////////////*版面配置 start*/////////////////////////////
      const svg = d3
        .select(`#${id}`)
      ...
      if (tooltips.open) {
        ...
        for (let i = 0; i < keyLen; i += 1) {
          ...
          if (showDate) {
            DateTextHeight = (fontSize + textMargin) * (i + 1) + dateMarginBottom
          }
          ...
          eltooltips
            ...
            .attr(
              'y',
              fontStack + paddinMarginStack + valueInTextMarginModePosition + DateTextHeight
            )
            .attr('fill', textColor)
        ...
        svg.selectAll('.hoverZone').on('mousemove', function trigger(event) {
          ...
          const dateStr = `${formateDate(date)} / ${formateMonth(date)}  ${formateYear(date)}`
          tooltipsTime.text(dateStr)
          const f = d3.format('$,')
          function getX() {
            return (
              e.x - elLeft - 2 * ttpadding - (isOverHalf ? thisToolTipsWidth + 2 * ttpadding : 0)
            )
          }
          ...
          eltooltips.selectAll('.tooltipsValue').each(function setValue(d, i) {
            ...
            const fp = d3.format(`.${p}%`)
            let percentage = 0
            if (thisData[allKey[i - 1]]) {
              percentage = thisData[key] / (thisData[key] + thisData[allKey[i - 1]])
            }
            if (thisData[allKey[i + 1]]) {
              percentage = thisData[key] / (thisData[key] + thisData[allKey[i + 1]])
            }
            d3.select(this).text(`${fp(percentage)} ( ${f(thisData[key])} )`)
          })
          eltooltips.selectAll('.tooltipsDot').style('fill', function setColor(d, i) {
            const color = colors[i]
            return color
          })
          // 寬度重新調整
          eltooltips.select('.tooltips-rect').attr('width', function getTooltipWidth() {
            ...
            const autoWidth = valueFontsLength + nameFontsLength + dot + ttpadding
            if (minWidth >= autoWidth) {
              svg.selectAll('.tooltipsValue').attr('x', 0 + minWidth - right)
              return minWidth
            }
            svg.selectAll('.tooltipsValue').attr('x', 0 + autoWidth - right + textMargin)
            return autoWidth + textMargin
          })
          // 高度重新調整
          eltooltips.select('.tooltips-rect').attr('height', function getTooltipWidth() {
            const elId = document.getElementById(id)
            const eltooltipsName = elId.querySelector(`.tooltipsName`)
            const eltooltipsTime = elId.querySelector(`.tooltipsTime`)
            const nameTextHeight = eltooltipsName.getBoundingClientRect().height
            const timeTextHeight = eltooltipsTime.getBoundingClientRect().height
            const tooltipsTopBottom = bottom + top
            let DateTextHeight = 0
            const scale = keyLen * 1.5
            let valueInTextMarginAdjustment = 0
            if (showDate) {
              DateTextHeight = timeTextHeight + textMargin + fontSize + dateMarginBottom
            }
            if (layoutMode === 'valueInTextMargin') {
