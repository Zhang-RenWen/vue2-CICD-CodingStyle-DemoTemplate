...
const events = ['resize', 'scroll', 'touchstart', 'touchend', 'pageshow', 'load']
...
const getBindingConfig = binding => {
  ...
  if (params.top) {
    result.offset.top = params.top
  }
  if (params.bottom) {
    result.offset.bottom = params.bottom
    result.side = 'bottom'
  }
  if (params.zIndex) {
    result.zIndex = params.zIndex
  }
  if (params.onStick) {
    result.onStick = params.onStick
  }
  return result
}
...
class Sticky {
  constructor(el, vm, binding) {
    ...
    const { offset, side, zIndex, onStick } = getBindingConfig(binding)
    this.options = {
  ...
  isBottomSticky() {
    ...
    const fromTop = window.innerHeight - this.state.placeholderElRect.top
  ...
  getHeight() {
    return this.placeholderEl.getBoundingClientRect().height
  }
...
export default {
  inserted(el, bind, vnode) {
    if (typeof bind.value === 'undefined' || bind.value) {
      el[namespace] = new Sticky(el, vnode.context, bind)
      el[namespace].doBind()
    }
  },
  unbind(el) {
